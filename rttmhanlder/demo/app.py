...
import gradio as gr
from gradio_rttmhanlder import RTTMHandler
...
with gr.Blocks() as demo:
    ...
    pipeline_selector = gr.Dropdown(choices=pipelines, value=pipelines[0], label="Choose the pipeline to apply", interactive=True)
    annotated_audio = AnnotatedAudio(type="filepath", interactive=True)
    rttm_handler = RTTMHandler()

    run_btn = gr.Button("Run pipeline")
    run_btn.click(fn=apply_pipeline, inputs=[pipeline_selector, annotated_audio], outputs=[annotated_audio, rttm_handler])
